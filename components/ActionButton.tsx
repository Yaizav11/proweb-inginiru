  ...
  return (
    <div className={`relative inline-flex flex-col gap-3 ${isFullWidth ? 'w-full' : ''}`}>
      {!showOptions ? (
        ...
        <div className="flex flex-col gap-3 animate-in fade-in slide-in-from-top-4 duration-300 w-full">
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            <button 
              ...
              className="bg-slate-200 text-slate-500 p-5 rounded-2xl hover:bg-slate-300 transition-colors flex items-center justify-center sm:w-16"
              title="Cancelar"
            >
              <X size={24} />
              <span className="sm:hidden font-bold uppercase text-xs ml-2">Cerrar</span>
            </button>
          </div>
        </div>
      )}
